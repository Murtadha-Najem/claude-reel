...
from .common import PY, RECORDS, ROOT, fmt_time
...
def write(meta, duration, video, audio, speech, decisions):
    ...
    if overview:
        ...
        look = f'"{PY}" "{ROOT / "look.py"}"'
        L += ["", "## Look further", "",
              f"- sheet of a time range: `{look} {code} sheet --start S --end E --n N` (N up to 24)",
              f"- one full frame, for reading text: `{look} {code} frame SECONDS`"]
    else:
